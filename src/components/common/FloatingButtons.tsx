...
export default function FloatingButtons() {
  ...
  return (
    <>
      <div className="fixed right-6 bottom-8 md:right-8 md:bottom-12 flex flex-col items-center z-[9999] bg-white/40 backdrop-blur-md shadow-sm hover:shadow-xl transition-shadow duration-300 border border-white/40 rounded-full py-3 px-2.5">
        <div className="flex flex-col items-center gap-2">
          {/* Chat Icon */}
          <button onClick={openChat} className="text-[#232589] hover:text-[#EEAB1B] transition-colors" aria-label="Customer Service">
          ...
          {/* Kakao Text */}
          <button onClick={openKakao} className="text-[#232589] hover:text-[#EEAB1B] transition-colors" aria-label="Kakao Channel">
            <span className="text-[11px] font-bold tracking-wide">kakao</span>
          </button>
        </div>

        {/* Go up Text */}
        <div className={`grid transition-all duration-300 ${isVisible ? 'grid-rows-[1fr] opacity-100 mt-2' : 'grid-rows-[0fr] opacity-0 mt-0 pointer-events-none'}`}>
          <div className="overflow-hidden">
            <button 
              onClick={scrollToTop}
              className="text-[#232589] hover:text-[#EEAB1B] transition-colors flex items-center justify-center pt-1"
            >
              <span 
                className="font-semibold tracking-widest flex items-center justify-center text-xs"
